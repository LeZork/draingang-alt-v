...
function handleCameraRotation(oldHeading, direction, speed) {
    const newHeading = oldHeading + (direction * speed);
...
function handleControls() {
    ...
    // Плавное вращение
    if (native.isDisabledControlPressed(0, 24) && _x > width / 2 - 100) {
        const rotationSpeed = 2;
        
        if (native.getDisabledControlNormal(0, 1) > 0.13) {
            oldHeading = handleCameraRotation(oldHeading, 1, rotationSpeed);
        }
    ...
    // Добавим плавное приближение/отдаление
    if (native.isDisabledControlPressed(0, 15) || native.isDisabledControlPressed(0, 16)) {
        const zoomSpeed = 0.5;
        const targetFov = native.isDisabledControlPressed(0, 15) ? Math.max(fov - zoomSpeed, 10) : Math.min(fov + zoomSpeed, 130);
