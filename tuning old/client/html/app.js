var app = new Vue({
    ...
    return{
        ...
        currentMods: [] // Хранение текущих модификаций для сравнения
    }
    },
    computed: {
        totalPrice() {
            return this.mods.reduce((total, mod) => {
                return total + (mod.value * mod.price);
            }, 0);
        }
    },
    watch: {
    ...
    vehicle_load(data, _name){
      this.name = _name
      this.mods = data
      this.currentMods = data.map(mod => ({ ...mod })); // Копируем текущие модификации для сравнения
    ...
    change_data(id, type){
        this.mods.map((mod, index) => {
            if (id === index){
                ...
                // Увеличиваем totalPrice только если значение изменилось
                if (mod.value !== this.currentMods[index].value) {
                    this.$set(mod, 'price', mod.price); // Обновляем цену
                } else {
                    this.$set(mod, 'price', 0); // Убираем цену, если значение не изменилось
                }
            }
        })
    ...
    applyChanges(){
        if ('alt' in window) {
            alt.emit('CU::Mods:Apply', this.mods, this.totalPrice)
        }
    },
    closeMenu() {
        // Логика закрытия меню
        if ('alt' in window) {
            alt.emit('CU::CloseTuning')
        }
    }
    }
})